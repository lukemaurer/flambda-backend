...
(** A list of values of specified types. For example, [[1; true; "hello"] :
    (int -> bool -> string -> 'r, 'r) t]. ['r] should be left polymorphic
    (a la [format3] and friends). *)
type ('a, 'r) t =
  | [] : ('r, 'r) t
  | ( :: ) : 'a * ('b, 'r) t -> ('a -> 'b, 'r) t
...
module Of (T : sig
  ...
  (** A list of values, each of which has type ['a T.t] for different ['a]. This
      ...
      lists to have the same element type. For example, [[Some 1; Some true; None]
      : (int -> bool -> string -> 'r, 'r) Of(Option).t]. *)
  type ('a, 'r) t =
    | [] : ('r, 'r) t
    | ( :: ) : 'a T.t * ('b, 'r) t -> ('a -> 'b, 'r) t
end

module Map (From : sig type 'a t end) (Into : sig type 'a t end) : sig
