...
let check0 ?(n = 1000) ?seed t ~type_ ~f ~name =
  ...
  while Outcome.is_success !outcome && !i <= n do
    ...
    | Success -> incr i
    | Failure failure ->
      let print_seq print_elt ppf seq =
        let rec take n seq () =
          if n <= 0
          then Seq.Nil
          else
        ...
        in
        Printer.list print_elt ppf (seq |> take 100 |> List.of_seq)
      in
      let seq = Type.shrink type_ repr in
      Format.eprintf "Shrinking %a:@.%a@." (Type.print type_) repr
        (print_seq (Type.print type_))
        seq;
  ...
  (* then begin Format.eprintf "FAILED after %d/%d attempts@." !i n;
     Format.eprintf "@[<hov 2>Counterexample:@ %a@]@." Type.Repr.print a; failed
     := true; t.something_has_failed <- true end else incr i done; *)
