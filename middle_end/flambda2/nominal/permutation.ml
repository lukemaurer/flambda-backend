...
module Make (N : Container_types.S) = struct
  ...
  (* CR-someday lmaurer: Define [N.Map.left_union] so we don't have this Some
      silliness. *)
  let left_union map1 map2 =
    N.Map.union (fun _k l _r -> Some l) map1 map2

  let compose ~second ~first =
    (* Find the triples [n1, n2, n3] where [first n1 = n2] and [second n2 = n3]. *)
    let chained =
      N.Map.inter (fun _n2 n1 n3 -> n1, n3) first.backwards second.forwards
    in
    (* Take the union of the forward directions, taking the first in case of
       a collision (since the first is the one that will actually act on the
       key) *)
    ...
    (* Add a correction for each chained triple [n1, n2, n3]. The new forward
       ...
       time. *)
    let forwards = N.Map.fold (fun _ (n1, n3) -> add_to_map n1 n3) chained forwards in
    ...
    let backwards = N.Map.fold (fun _ (n1, n3) -> add_to_map n3 n1) chained backwards in
    { forwards; backwards }
